...
const availableModels = 'gemini-2.5-flash、gemini-2.5-pro'.split('、').map((it) => it.trim());
            ...
            >
                ...
                <div className='config-item'>
                    <Input
                        ...
                        value={geminiConfig[INSTANCE_NAME_CONFIG_KEY]}
                        variant='bordered'
                        classNames={{
                ...
                <div className='config-item'>
                    <h3 className='my-auto'>{t('services.translate.geminipro.model')}</h3>
                    <Dropdown>
                        <DropdownTrigger>
                            <Button variant='bordered'>{geminiConfig.model}</Button>
                        </DropdownTrigger>
                        <DropdownMenu
                            autoFocus='first'
                            aria-label='model'
                ...
                <div className='bg-content2 rounded-[10px] p-3'>
                    {geminiConfig.promptList &&
                        geminiConfig.promptList.map((prompt, index) => {
                            return (
                                <div className='config-item'>
                                    <Textarea
                                        label={prompt.role}
                        ...
                        onPress={() => {
                            setServiceConfig({
                                ...
                                promptList: [
                                    ...
                                    {
                                        role:
                                            geminiConfig.promptList.length === 0
                                                ? 'system'
                                                : geminiConfig.promptList.length % 2 === 0
                                                  ? 'assistant'
                                                  : 'user',
                                        content: '',
                                    },
