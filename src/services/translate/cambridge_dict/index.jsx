...
function render(node) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE && node.tagName === 'A' && node.classList.contains('query')) {
        return node.textContent;
    }
    ...
    if (node.nodeType === Node.ELEMENT_NODE && (node.tagName === 'STRONG' || node.tagName === 'SPAN')) {
...
export async function translate(text, from, to) {
    ...
    const resultMap = [...entryNodes].reduce((dict, entryNode) => {
        ...
        const explanations = [...defBlockNodes]
            .filter((defBlockNode) => {
                // If node has attribute data-wl-senseid and contains panel, skip it
                return (
                    !defBlockNode.hasAttribute('data-wl-senseid') ||
                    !defBlockNode.getAttribute('data-wl-senseid').includes('panel')
                );
            })
            .map((defBlockNode) => {
                const engDef = defBlockNode
                ...
                const explains = [
                    engDef,
                    ...defBlockNode
                        .querySelector('.def-body.ddef_b .trans.dtrans.dtrans-se.break-cj')
                        .innerText.replace(/\s+/g, ' ')
                        .trim()
                        .split(';'),
                ];
        ...
        const sentences = [...defBlockNodes]
            ...
            .map((defBlockNode) => {
                ...
                const result = Array.from(defBlockNode.querySelector('div.examp span.eg').childNodes)
                    .map(render)
                    .join('')
                    .trim();
